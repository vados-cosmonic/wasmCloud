...
impl Provider for KafkaMessagingProvider {
    ...
    async fn receive_link_config_as_target(
        ...
        let mut consumer = AsyncKafkaConsumer::from_async_client(client, |b| {
            b.with_topic(topic.into())
        }).await.with_context(|| {
            warn!(
                source_id,
        ...
        // TODO: Handle initial start offset
        // TODO: handle max wait milliseconds (100) -- use it as a point to yield?
    ...
    async fn shutdown(&self) -> Result<()> {
        // self.connections
        //     .write()
        //     .context("failed to get write access to connections")?
        //     .drain()
        //     .for_each(|(_source_id, connection)| {
        //         if let (consumer, stop_tx) = *connection.consumer {
        //             stop_tx.send(());
...
impl exports::wasmcloud::messaging::consumer::Handler<Option<Context>> for KafkaMessagingProvider {
    ...
    async fn publish(
        ...
        // // Retrieve a list of hosts for the given connection
        // let hosts = {
        ...
        let connections = self.connections.read().await;
        let Some(KafkaConnection { client, .. }) = connections.get(component_id) else {
        ...
        // TODO: How often should be reload metadata? the topic list here could be stale
        ...
        if !topics.contains(&msg.subject) {
            // **PROBLEM**: there's no way to create topics with kafka-rs***
            // We'd have to use *both* rskafka and kafka-rs to support topic creation.
