...
impl WashDevSession {
    ...
    async fn start_host(
        ...
        .await
        .context("failed to start NATS server for wash dev")?;
        ...
        .await
        .context("failed to start wadm for wash dev")?;
        ...
        Ok((Some(nats_child), Some(wadm_child), Some(wasmcloud_child)))
    }
}
        ...
        select! {
            ...
            _ = stop_rx.recv() => {
                ...
                if !cmd.leave_host_running {
                    ...
                    // Stop host
                    wasmcloud_child
                        .context("missing wasmcloud process")?
                        .kill()
                        .await
                    ...
                    // Stop WADM
                    wadm_child
                        .context("missing wadm child process")?
                        .kill()
                        .await
                        .context("failed to stop wadm child process")?;
                    ...
                    // Stop NATS
                    nats_child.context("missing NATS child process")?.kill().await?;
                }
